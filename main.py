...
class QRCodeGenerator(QMainWindow):
    ...
    def initUI(self):
        ...
        self.pdf_button = QPushButton("Generate QR with PDF", self)
        ...
        self.pdf_button.clicked.connect(self.generate_qr_with_pdf)
    ...
    def generate_qr_with_url_and_pdf(self, url, pdf_file_path):
        ...
        self.generate_qr_code(qr_url_data, "url_qr_code.png")
        self.attach_qr_to_pdf(pdf_file_path, "pdf_url_qr_code.png", "url_qr_code.png")

    def attach_qr_to_pdf(self, pdf_file_path, qr_pdf_path, qr_url_path):
        output_file_path = f"{pdf_file_path.rsplit('.', 1)[0]}_with_QR.pdf"
        add_qr_to_pdf(qr_pdf_path, qr_url_path, pdf_file_path, output_file_path)
        QMessageBox.information(
            self,
            "Success",
            f"QR codes generated and attached to PDF: {output_file_path}",
        )
...
def add_qr_to_pdf(
    qr_pdf_image_path, qr_url_image_path, pdf_file_path, output_file_path
):
    # Load the QR code images
    qr_pdf_image = Image.open(qr_pdf_image_path)
    qr_url_image = Image.open(qr_url_image_path)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        for page_num in range(total_pages):
            ...
            # Add the QR code images to the canvas
            qr_width = 50
            qr_height = 50
            ...
            # Merge the original page and canvas with the QR code images
